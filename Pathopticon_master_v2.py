...
def input_paths(proj_path):
    ...
    input_paths_dict = {'L1000_gene_info_path': proj_path + 'Pathopticon_external_data/L1000/' + 'GSE92742_Broad_LINCS_gene_info.txt',
                        ...
                        'pos_edges_dict_path': proj_path + 'ALL_QUIZC75/' + 'ALL_cell_pos_edges_dict_0.75.pickle',
                        'neg_edges_dict_path': proj_path + 'ALL_QUIZC75/' + 'ALL_cell_neg_edges_dict_0.75.pickle',
                        'drugs_dict_path': proj_path + 'ALL_QUIZC75/' + 'ALL_cell_drugs_dict_0.75.pickle',     
                        'MODZ_networks_path': proj_path + 'ALL_MODZ_networks/',
                        'CD_networks_path': proj_path + 'ALL_CD_networks/',
...
# def get_MSigDB_geneset_targets_CTD(cgp_updn, CTD_mRNA_up_final, CTD_mRNA_dn_final):
# 
#     geneset_pert_iname_dict = {}
#     for geneset_name in tqdm(cgp_updn_labels, position=0, leave=True):
#        
#         geneset_pert_iname_dict[geneset_name] = (set(CTD_mRNA_up_final[CTD_mRNA_up_final['GeneSymbol']
#                                                                        .isin(cgp_updn['%s_UP' % geneset_name])]['pert_iname']) | 
#                                                  set(CTD_mRNA_dn_final[CTD_mRNA_dn_final['GeneSymbol']
#                                                                        .isin(cgp_updn['%s_DN' % geneset_name])]['pert_iname']))
# 
#     return geneset_pert_iname_dict
...
# def input_paths_rev(proj_rev_path):
# 
#     input_paths_dict_rev = {'QUIZC_cids_inchi_smiles_path': proj_rev_path + 'REV_Pathopticon_intermediary_outputs/' + 'QUIZC_drug_CIDs_inchi_smiles.csv',
#                         'tool_path': proj_rev_path + 'REV_Pathopticon_intermediary_outputs/' + 'QUIZC_activityStats_nooutliers_df_besttool.csv',
#                         'benchmark_genesets_path': proj_rev_path + 'FE_adj_100_benchmark_genesets.csv',
#                         
#                         'CTD_path': proj_rev_path + 'CTD_chem_gene_ixns.tsv',
#                         'CTD2CID_path': proj_rev_path + 'CTD2CID.csv',
#                         
#                         'FE_adj_pval_UP_UP_path': proj_rev_path + 'FE_table_adj_pvals_UP_UP.csv',
#                         'FE_adj_pval_DN_DN_path': proj_rev_path + 'FE_table_adj_pvals_DN_DN.csv',
#                         'FE_adj_pval_UP_DN_path': proj_rev_path + 'FE_table_adj_pvals_UP_DN.csv',
#                         'FE_adj_pval_DN_UP_path': proj_rev_path + 'FE_table_adj_pvals_DN_UP.csv',
#                         
#                         'pos_edges_dict_path': proj_rev_path + 'ALL_QUIZC75/' + 'ALL_cell_pos_edges_dict_0.75.pickle',
#                         'neg_edges_dict_path': proj_rev_path + 'ALL_QUIZC75/' + 'ALL_cell_neg_edges_dict_0.75.pickle',
#                         'drugs_dict_path': proj_rev_path + 'ALL_QUIZC75/' + 'ALL_cell_drugs_dict_0.75.pickle',     
#                         'MODZ_networks_path': proj_rev_path + 'ALL_MODZ_networks/',
#                         'CD_networks_path': proj_rev_path + 'ALL_CD_networks/'
#                         
#                         }
#     
#     return input_paths_dict_rev


def get_MSigDB_geneset_targets_CTD_FE(cgp_updn, cgp_updn_labels, FE_adj_pval_UP_path, FE_adj_pval_DN_path, adj_pval_threshold=0.05):

    FE_table_adj_pvals_UP = pd.read_csv(FE_adj_pval_UP_path, index_col=0)
    FE_table_adj_pvals_DN = pd.read_csv(FE_adj_pval_DN_path, index_col=0)
    
    geneset_pert_iname_dict = {}
    for geneset_name in tqdm(cgp_updn_labels, position=0, leave=True):
       
        geneset_pert_iname_dict[geneset_name] = (set(FE_table_adj_pvals_UP.loc[geneset_name][FE_table_adj_pvals_UP.loc[geneset_name]<adj_pval_threshold].index) |
                                                 set(FE_table_adj_pvals_DN.loc[geneset_name][FE_table_adj_pvals_DN.loc[geneset_name]<adj_pval_threshold].index))
...
def calculate_FE_pvals(cgp_updn, cgp_updn_labels, CTD_mRNA_up_final, CTD_mRNA_dn_final, QUIZC_allgenes, QUIZC_alldrugs, 
                       cgp_direction, CTD_direction, proj_path):

    if not os.path.exists(proj_path +  'FE_table_pvals_%s_%s.csv' % (cgp_direction, CTD_direction)):
        ...
        FE_table_pvals = pd.DataFrame(index=set(cgp_updn_labels), columns=QUIZC_CTD_drugs)
        FE_table_adj_pvals = pd.DataFrame(index=set(cgp_updn_labels), columns=QUIZC_CTD_drugs)

        if CTD_direction == 'UP':

            for i in tqdm(set(cgp_updn_labels), position=0, leave=True):
                for j in QUIZC_CTD_drugs:
                    A = len(set(cgp_updn['%s_%s' % (i, cgp_direction)]) & set(CTD_mRNA_up_final[CTD_mRNA_up_final['pert_iname']==j]['GeneSymbol']))
                    B = len(set(cgp_updn['%s_%s' % (i, cgp_direction)]) - set(CTD_mRNA_up_final[CTD_mRNA_up_final['pert_iname']==j]['GeneSymbol']))
                    C = len(set(CTD_mRNA_up_final[CTD_mRNA_up_final['pert_iname']==j]['GeneSymbol']) - set(cgp_updn['%s_%s' % (i, cgp_direction)]))
                    D = len(set(QUIZC_allgenes['Gene_symbol']) - (set(cgp_updn['%s_%s' % (i, cgp_direction)]) | set(CTD_mRNA_up_final[CTD_mRNA_up_final['pert_iname']==j]['GeneSymbol'])))
                    FE_table_pvals.at[i, j] = st.fisher_exact([[A, B], [C, D]], alternative='greater')[1]

            FE_table_pvals.to_csv(proj_path +  'FE_table_pvals_%s_%s.csv' % (cgp_direction, CTD_direction))
         
        elif CTD_direction == 'DN':
            
            for i in tqdm(set(cgp_updn_labels), position=0, leave=True):
                for j in QUIZC_CTD_drugs:
                    A = len(set(cgp_updn['%s_%s' % (i, cgp_direction)]) & set(CTD_mRNA_dn_final[CTD_mRNA_dn_final['pert_iname']==j]['GeneSymbol']))
                    B = len(set(cgp_updn['%s_%s' % (i, cgp_direction)]) - set(CTD_mRNA_dn_final[CTD_mRNA_dn_final['pert_iname']==j]['GeneSymbol']))
                    C = len(set(CTD_mRNA_dn_final[CTD_mRNA_dn_final['pert_iname']==j]['GeneSymbol']) - set(cgp_updn['%s_%s' % (i, cgp_direction)]))
                    D = len(set(QUIZC_allgenes['Gene_symbol']) - (set(cgp_updn['%s_%s' % (i, cgp_direction)]) | set(CTD_mRNA_dn_final[CTD_mRNA_dn_final['pert_iname']==j]['GeneSymbol'])))
                    FE_table_pvals.at[i, j] = st.fisher_exact([[A, B], [C, D]], alternative='greater')[1]
        ...
        FE_table_adj_pvals.to_csv(proj_path +  'FE_table_adj_pvals_%s_%s.csv' % (cgp_direction, CTD_direction))
    
    else:
        
        print('The Fisher\'s Exact p-value table exists in this directory.')
        

def import_Harmonizome(Harmonizome_up_path, Harmonizome_dn_path, separate_updn=False):
    
    if separate_updn == False:
        
        Harmonizome_updn = {}
        Harmonizome_updn_labels = []

        f = open(Harmonizome_up_path)
        for line in f:
            line = line.rstrip()
            d = line.split('\t')[0].split('_GSE')[0]
            g = line.split('\t')[2:]
            Harmonizome_updn['%s_UP' % d] = g   
            Harmonizome_updn_labels.append(d)
        f.close()

        f = open(Harmonizome_dn_path)
        for line in f:
            line = line.rstrip()
            d = line.split('\t')[0].split('_GSE')[0]
            g = line.split('\t')[2:]
            Harmonizome_updn['%s_DN' % d] = g       
        f.close()

        return Harmonizome_updn, Harmonizome_updn_labels
        ...
        for line in f:
            ...
            d = line.split('\t')[0].split('_GSE')[0]
            g = line.split('\t')[2:]
            Harmonizome_dn['%s' % d] = g       
        f.close()       
        
        return Harmonizome_up, Harmonizome_dn
        
def run_benchmark_ALLcellsCTD_FE(proj_path, benchmark_path, method_name, 
                                 PACOS_Spearman_threshold=10, r=2.0,  Nrand=100, auc_params={'auc_threshold':5, 'binsize':1}, 
                                 models=['PACOS_Spearman_rho', 'PACOS_Spearman_rho_reverse', 
    ...
    geneset_pert_iname_dict_fwd = get_MSigDB_geneset_targets_CTD_FE(cgp_updn, cgp_updn_labels, 
                                                                    input_paths_dict['FE_adj_pval_UP_UP_path'], 
                                                                    input_paths_dict['FE_adj_pval_DN_DN_path'])
    geneset_pert_iname_dict_rev = get_MSigDB_geneset_targets_CTD_FE(cgp_updn, cgp_updn_labels, 
                                                                    input_paths_dict['FE_adj_pval_UP_DN_path'], 
                                                                    input_paths_dict['FE_adj_pval_DN_UP_path'])
    ...
    for geneset_name in tqdm(benchmark_genesets['Geneset_name'].to_numpy(), position=0, leave=True):
        ...
        if not os.path.exists(benchmark_path +  'Nested_%s_%s_r%s.csv' % (method_name, geneset_name, int(r))):
            ...
            pacos_tool_merged_df = PACOS(pcp_geneset_df, pcp_perturbation_df_dict, alldrugs, allcells,
                                         ...
                                         benchmark_path, 
                                         method_name=method_name, geneset_name=geneset_name, 
                                         r=r, threshold=PACOS_Spearman_threshold, tqdm_off=True)
            ...
            rand_model_auroc_df_dict, rand_model_auprc_df_dict = PACOS_cell_AUC_randomize_FE(pacos_tool_merged_df, allcells, 
                                                                                             geneset_pert_iname_dict_fwd[geneset_name], 
                                                                                             geneset_pert_iname_dict_rev[geneset_name],
                                                                                             models=models, auc_params=auc_params, 
                                                                                             Nrand=Nrand, tqdm_off=True) 
